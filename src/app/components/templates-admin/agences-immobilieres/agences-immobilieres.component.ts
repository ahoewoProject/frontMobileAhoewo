...
export class AgencesImmobilieresComponent implements OnInit {
  ...
  modifierAgenceImmobiliere(): void {
    this.agenceImmobiliereData.append('logoAgence', this.logoAgence);
    this.agenceImmobiliereData.append('agenceImmobiliereJson', JSON.stringify(this.agenceImmobiliere));

    this.agenceImmobiliereService.updateAgenceImmobiliere(this.agenceImmobiliere.id, this.agenceImmobiliereData).subscribe(
      (response) =>{
        if(response.id > 0) {
          this.voirListe();
          ...
          this.afficherFormulaireModifier(this.agenceImmobiliere.id);
        }
    },
    (error) =>{
      console.log(error)
